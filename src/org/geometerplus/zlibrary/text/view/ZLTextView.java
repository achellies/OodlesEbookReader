...
import org.geometerplus.android.fbreader.ManageBookmarks;
...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	private void drawSelectionCursor(ZLPaintContext context,
			ZLTextSelection.Point pt, int direction) {
		if (pt == null) {
			return;
		}
		...
		fbreader.runAction(ActionCode.DRAW_POINTER,context,pt.X,pt.Y,direction);	
//		context.drawImage(context,pt,direction);
		/*final int w = ZLTextSelectionCursor.getWidth() / 2;
		final int h = ZLTextSelectionCursor.getHeight();
		...
		context.setFillColor(context.getBackgroundColor(), 255);
	...
	public synchronized void paint(ZLPaintContext context, PageIndex pageIndex) {
		...
		drawSelectionCursor(context,
				getSelectionCursorPoint(page, ZLTextSelectionCursor.Left),1);
		drawSelectionCursor(context,
				getSelectionCursorPoint(page, ZLTextSelectionCursor.Right),0);
	}
	...
	private void drawBackgroung(ZLTextAbstractHighlighting highligting,
	...
	private void drawTextLine(ZLTextPage page, ZLTextLineInfo info, int from,
			int to, int y) {
		drawBackgroung(mySelection, getSelectedBackgroundColor(), page, info,
				from, to, y);
		drawBackgroung(myHighlighting, getHighlightingColor(), page, info,
				from, to, y);
			...
			if (element == area.Element) {
				...
				if (element instanceof ZLTextWord) {
					drawWord(
							...
							-1,
							false,
							mySelection.isAreaSelected(area) ? getSelectedForegroundColor()
	...
	public final synchronized void gotoPosition(int paragraphIndex,
		...
		if (myModel != null && myModel.getParagraphsNumber() > 0) {
			...
			Log.i("page number or move through page", paragraphIndex+"");
	...
	/*private void checkBookmark() {
		...
		{
			fbreader.runAction(ActionCode.PAGE_BOOKMARKED);
			
		}
		else
		{
			fbreader.runAction(ActionCode.PAGE_NOT_BOOKMARKED);
		}
	...
	private void skip(ZLTextWordCursor cursor, int unit, int size) {
		...
		while (!cursor.isEndOfParagraph() && (size > 0)) {
			ZLTextLineInfo info = processTextLine(paragraphCursor,
	...
	public boolean canScroll(PageIndex index) {
		...
		case next: {
			...
			return cursor != null
					&& !cursor.isNull()
					&& (!cursor.isEndOfParagraph() || !cursor
							.getParagraphCursor().isLast());
		}
		case previous: {
			final ZLTextWordCursor cursor = getStartCursor();
			return cursor != null
					&& !cursor.isNull()
					&& (!cursor.isStartOfParagraph() || !cursor
							.getParagraphCursor().isFirst());
		}
		}
	}
}
