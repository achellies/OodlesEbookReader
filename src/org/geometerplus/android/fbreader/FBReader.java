...
import org.geometerplus.android.fbreader.oodlesui.ListArrayAdapter;
import org.geometerplus.android.fbreader.oodlesui.UiButtonsEventHandler;
import org.geometerplus.android.fbreader.oodlesui.UiListItemEventHandler;
...
public final class FBReader extends ZLAndroidActivity {
	...
	private final BroadcastReceiver myPluginInfoReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			final ArrayList<PluginApi.ActionInfo> actions = getResultExtras(true).<PluginApi.ActionInfo> getParcelableArrayList(
					PluginApi.PluginInfo.KEY);
			if (actions != null) {
				synchronized (myPluginActions) {
					final FBReaderApp fbReader = (FBReaderApp) FBReaderApp.Instance();
					// fbReader.Model.Book.getTitle()
					int index = 0;
					...
					for (PluginApi.ActionInfo info : myPluginActions) {
						fbReader.addAction(PLUGIN_ACTION_PREFIX + index++, new RunPluginAction(FBReader.this, fbReader, info.getId()));
					}
				}
			}
		}
	...
	public void onCreate(Bundle icicle) {
		...
		fbReader.addAction(ActionCode.TOUCH_ACTIONS, new TouchScreenAction(this, fbReader, eventHandler));
		...
		fbReader.addAction(ActionCode.SET_SCREEN_ORIENTATION_SYSTEM, new SetScreenOrientationAction(this, fbReader,
				ZLibrary.SCREEN_ORIENTATION_SYSTEM));
		fbReader.addAction(ActionCode.SET_SCREEN_ORIENTATION_SENSOR, new SetScreenOrientationAction(this, fbReader,
				ZLibrary.SCREEN_ORIENTATION_SENSOR));
		fbReader.addAction(ActionCode.SET_SCREEN_ORIENTATION_PORTRAIT, new SetScreenOrientationAction(this, fbReader,
				ZLibrary.SCREEN_ORIENTATION_PORTRAIT));
		fbReader.addAction(ActionCode.SET_SCREEN_ORIENTATION_LANDSCAPE, new SetScreenOrientationAction(this, fbReader,
				ZLibrary.SCREEN_ORIENTATION_LANDSCAPE));
		if (ZLibrary.Instance().supportsAllOrientations()) {
			fbReader.addAction(ActionCode.SET_SCREEN_ORIENTATION_REVERSE_PORTRAIT, new SetScreenOrientationAction(this, fbReader,
					ZLibrary.SCREEN_ORIENTATION_REVERSE_PORTRAIT));
			fbReader.addAction(ActionCode.SET_SCREEN_ORIENTATION_REVERSE_LANDSCAPE, new SetScreenOrientationAction(this, fbReader,
	...
	private void setupViews() {
		// Log.d(TAG,"setupViews Method executed");
		eventHandler = new UiButtonsEventHandler(this);
		findViewById(R.id.fb_reader_brightness_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_effect_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_text_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_orientation_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_page_scroll_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_action_search_btn).setOnClickListener(eventHandler);

		findViewById(R.id.fb_reader_decrease_font_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_increase_font_button).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_bookmark_ribbon).setOnClickListener(eventHandler);
		findViewById(R.id.fb_reader_action_toc_btn).setOnClickListener(eventHandler);

		findViewById(R.id.fb_reader_action_setting_btn).setOnClickListener(eventHandler);

	}

	private void setlistView() {
		// Log.d(TAG,"setlistView Method executed");

		ListView effect_ListView = (ListView) findViewById(R.id.reader_page_effect_mode_listview);
		ArrayList<String> planetList = new ArrayList<String>();
		planetList.add("Sepia");
		planetList.add("Night");
		planetList.add("Classic");
		ArrayAdapter<String> listAdapter = new ListArrayAdapter(this, planetList, 0);
		effect_ListView.setAdapter(listAdapter);
		effect_ListView.setOnItemClickListener(new UiListItemEventHandler());

		ListView orientation_mode_ListView = (ListView) findViewById(R.id.reader_page_orientaion_mode_ListView);
		ArrayList<String> effect_List = new ArrayList<String>();
		effect_List.add("Auto");
		effect_List.add("Landscape");
		effect_List.add("Portrait");
		ArrayAdapter<String> effect_listAdapter = new ListArrayAdapter(this, effect_List, 1);
		orientation_mode_ListView.setAdapter(effect_listAdapter);
		orientation_mode_ListView.setOnItemClickListener(new UiListItemEventHandler());

	}

	private class CheckBookmark extends AsyncTask<Void, Boolean, Void> {
	...
	private boolean isBookmarked() {
		ManageBookmarks bookmarkManager = new ManageBookmarks();
		final FBReaderApp fbreader = (FBReaderApp) FBReaderApp.Instance();
